...
<div class="overflow-hidden">
	<div class="mb-4 flex flex-row flex-wrap items-center justify-between gap-4 dark:text-white">
		<div class="no-scrollbar overflow-auto ">
			<h1 class="text-3xl font-semibold">Editor for:</h1>
			<code class="overflow-hidden">{key}</code>
		</div>
	...
	<div class="flex flex-col gap-4">
		{#each translationState.translations as t}
			<div
				class="group rounded-xl bg-slate-100 p-4 transition hover:bg-slate-400 dark:bg-slate-700 dark:text-white hover:dark:bg-slate-400"
			>
