...
<div class="flex flex-col gap-2 container mx-auto dark:text-white">
	...
	<p class="mb-4">
		...
		find them here. 
		Click any result to jump to the editor-view.
	</p>

	<div class="flex sticky top-14 bg-slate-100 dark:bg-slate-700 shadow">
	...
	{#each dupes as [key, values]}
		<button
			...
			class="flex gap-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-400 dark:hover:bg-slate-400 transition cursor-pointer"
		>
			...
			<div class="flex flex-col gap-2 w-4/5 p-2 text-left">
				<ul>
					{#each values.dupes as duplicate}
						<li>{duplicate} <span class="text-slate-300 dark:text-slate-600">({values.locale})</span></li>
					{/each}
				</ul>
			</div>
		</button>
